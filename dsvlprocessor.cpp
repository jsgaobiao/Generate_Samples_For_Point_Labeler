...
void DsvlProcessor::ProcessOneFrame(int fno)
{
    int x, y;
    double	rng;
    point3fi *p;
    ...
    rm.regnum = 0;
    rm.ang = onefrm->dsv[0].ang;
    rm.shv = onefrm->dsv[0].shv;
    ...
    for (int i=0; i<BKNUM_PER_FRM; i++) {
        for (int j=0; j<LINES_PER_BLK; j++) {
            for (int k=0; k<PNTS_PER_LINE; k++) {
                ...
                int temp = rm.regionID[y*rm.wid+x];
                // 这句话好像没有用到
...
void DsvlProcessor::Processing()
{
    ...
    cv::namedWindow("gt_vis");
    cv::namedWindow("input_vis");
    cv::namedWindow("segmentation");
    cv::moveWindow("range image", 400, 30);
    cv::moveWindow("gt_vis", 400, 230);
    cv::moveWindow("input_vis", 400, 430);
    cv::moveWindow("segmentation", 400, 630);
